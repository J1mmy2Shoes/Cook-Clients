...
import com.example.deliverable1userinterface.accTypes.Cook;
...
public class CookPage extends AppCompatActivity {
    EditText Cook_firstName, Cook_lastName, Cook_email, Cook_addy, Cook_password, Cook_description;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerButton = findViewById(R.id.create_cook);
                ...
                authenticator.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            newCook(first, last, email, user.getUid());
                            Toast.makeText(CookPage.this, "Successfully created a customer account!", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(CookPage.this, welcomePage.class));
                            finishAffinity();
                        }
                        else {
    ...
    private void newCook(String first, String last, String email, String userID) {
        Cook cook = new Cook(first, last, email);
        mDatabase.child("users").child(userID).setValue(cook);
    }
}
